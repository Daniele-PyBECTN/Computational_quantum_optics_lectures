...
In this chapter we have seen how finite‐dimensional Hilbert spaces can be encoded with NumPy arrays, turning state kets into vectors and operators into matrices. We defined and manipulated  

- Pauli matrices for qubits,  
- the bosonic annihilation ($\hat{a}$) and creation ($\hat{a}^{\dagger}$) operators for a truncated harmonic oscillator,  
- Fock states and the action of $\hat{a}$ on these states,  
- and the partial trace to extract reduced density matrices from composite systems.

These building blocks demonstrate how fundamental quantum‐mechanical concepts translate into straightforward linear algebra routines in Python. With these tools you can now assemble more complex many‐body Hamiltonians, simulate time evolution, and analyze entanglement in multipartite systems. Future chapters will build on these foundations to explore more advanced topics in quantum mechanics and quantum information theory.
