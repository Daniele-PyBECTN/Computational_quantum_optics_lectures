...
When dealing with the time-dependent Schrödinger equation for a *time-independent* Hamiltonian $\hat{H}$, we can represent $\ket{\psi(t)}$ in a certain basis, turning the Schrödinger equation into: $$i \hbar \,\frac{d}{dt} \mathbf{c}(t) = H \,\mathbf{c}(t),$$ or equivalently,
$$
